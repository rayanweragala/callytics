...
describe("AsteriskConfigService", () => {
  ...
  it("generatePjsipConfig with extensions produces [endpoint/XXXX] sections with correct auth and aor blocks", () => {
    ...
    expect(config).toContain("transport = transport-udp");
  ...
  it("generatePjsipConfig with webrtc extension includes websocket transport and webrtc flags", () => {
    const ext = {
      username: "1234",
      password: "pwd",
    ...
    expect(config).toContain("transport = transport-ws");
    expect(config).toContain("webrtc = yes");
    expect(config).toContain("media_encryption = dtls");
    expect(config).toContain("rtcp_mux = yes");
  });
