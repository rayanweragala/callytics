...
import { FlowsService, validateEdgesConfig, validateNodeConfig, validateNodesConfig } from './flows.service';
...
describe('FlowsService', () => {
  ...
  describe('create', () => {
    it('should create a new flow', async () => {
      const dto = {
        name: 'New Flow',
        nodes: [{ nodeKey: 'start', type: 'start', config: { flow_default_timeout_ms: 10000 } }],
        edges: [],
      };
  ...
  describe('root flow tree versioning on save', () => {
    const changedSubflowDto = {
      ...
      nodes: [
        ...
        },
        {
          ...
          positionX: 120,
          positionY: 0,
          config: { prompt_audio_file_id: 1, branches: ['1'], timeout_ms: 5000 },
          groupId: null,
          subflowId: null,
        },
      ],
    ...
    it('saving a subflow creates a new version on the root flow, not the subflow', async () => {
      const subflow = { id: 3, parentFlowId: 2, parentNodeKey: 'menu_1', name: 'Subflow A', slug: 'subflow-a', createdAt: new Date(), updatedAt: new Date(), currentVersionId: 30 };
      const subflowVersion = { id: 30, flowId: 3, versionNumber: 1, snapshot: { nodes: [{ nodeKey: 'start', type: 'start', label: 'Start', positionX: 0, positionY: 0, config: {}, groupId: null, subflowId: null }], edges: [] }, createdAt: new Date() };
      const rootVersion = { id: 11, flowId: 1, versionNumber: 4, snapshot: { flowId: 1, name: 'Root', nodes: [], edges: [], subflows: [] }, createdAt: new Date() };
      const savedRootVersion = { id: 12, flowId: 1, versionNumber: 5, snapshot: { flowId: 1, name: 'Root', nodes: [], edges: [], subflows: [] }, createdAt: new Date() };
      ...
      mockManager.findOne.mockImplementation((entity, options) => {
        ...
        if (entity === FlowVersionEntity) {
          if (options?.where?.flowId === 3) return Promise.resolve(subflowVersion);
          return Promise.resolve(null);
        }
        return Promise.resolve(null);
      });
      ...
      mockManager.findOneOrFail.mockImplementation((entity, options) => {
        if (entity === CallFlowEntity && options?.where?.id === 1) {
          return Promise.resolve({ id: 1, currentVersionId: 11, name: 'Root', updatedAt: new Date() });
        }
      ...
      jest.spyOn(service as any, 'buildFlowDetail').mockResolvedValue({ id: 3 } as any);
      jest.spyOn(service as any, 'normalizeNodesForSave').mockResolvedValue(changedSubflowDto.nodes as any);
      jest.spyOn(service as any, 'getRootFlowId').mockResolvedValue(1);
      jest.spyOn(service as any, 'buildFullTreeSnapshot').mockResolvedValue({ flowId: 1, name: 'Root', nodes: [], edges: [], subflows: [{ flowId: 3, name: 'Subflow A', nodes: changedSubflowDto.nodes as any, edges: changedSubflowDto.edges as any }] });
      jest.spyOn(service as any, 'getLatestVersion').mockResolvedValue(rootVersion as any);
      const createStoredVersionSpy = jest.spyOn(service as any, 'createStoredVersion').mockResolvedValue(savedRootVersion as any);
      jest.spyOn(service as any, 'saveNodesAndEdges').mockResolvedValue(undefined);

      await service.update(3, changedSubflowDto as any);

      expect(createStoredVersionSpy).toHaveBeenCalledTimes(1);
      expect(createStoredVersionSpy).toHaveBeenCalledWith(expect.anything(), 1, 5, expect.any(Object), undefined);
      expect(createStoredVersionSpy).not.toHaveBeenCalledWith(expect.anything(), 3, expect.anything(), expect.anything(), expect.anything());
    });
    ...
    it('saving a subflow with no actual changes does not create a root version', async () => {
      ...
      const subflowVersion = { id: 30, flowId: 3, versionNumber: 1, snapshot: unchangedSnapshot, createdAt: new Date() };

      mockManager.findOne.mockImplementation((entity, options) => {
        if (entity === CallFlowEntity && options?.where?.id === 3) return Promise.resolve(subflow);
        if (entity === FlowVersionEntity && options?.where?.flowId === 3) return Promise.resolve(subflowVersion);
        return Promise.resolve(null);
      });
      mockManager.save.mockResolvedValue(subflow);
      mockManager.findOneOrFail.mockResolvedValue(subflowVersion);
      jest.spyOn(service as any, 'buildFlowDetail').mockResolvedValue({ id: 3 } as any);
      jest.spyOn(service as any, 'normalizeNodesForSave').mockResolvedValue(unchangedSnapshot.nodes as any);
      const createStoredVersionSpy = jest.spyOn(service as any, 'createStoredVersion').mockResolvedValue({ id: 999 } as any);
    ...
    it('saving a subflow with a node config change writes a root version with full tree snapshot', async () => {
      ...
      const previousSubflowSnapshot = {
        nodes: [
          { nodeKey: 'start', type: 'start', label: 'Start', positionX: 0, positionY: 0, config: {}, groupId: null, subflowId: null },
          { nodeKey: 'menu', type: 'menu', label: 'Menu', positionX: 120, positionY: 0, config: { prompt_audio_file_id: 1, branches: ['1'], timeout_ms: 5000 }, groupId: null, subflowId: null },
        ],
        edges: [{ sourceNodeKey: 'start', targetNodeKey: 'menu', branchKey: 'default', condition: null }],
      };
      const nextSubflowSnapshot = {
        nodes: [
          { nodeKey: 'start', type: 'start', label: 'Start', positionX: 0, positionY: 0, config: {}, groupId: null, subflowId: null },
          { nodeKey: 'menu', type: 'menu', label: 'Menu', positionX: 120, positionY: 0, config: { prompt_audio_file_id: 1, branches: ['1', '2'], timeout_ms: 5000 }, groupId: null, subflowId: null },
        ],
        edges: [{ sourceNodeKey: 'start', targetNodeKey: 'menu', branchKey: 'default', condition: null }],
      };
      const subflowVersion = { id: 30, flowId: 3, versionNumber: 1, snapshot: previousSubflowSnapshot, createdAt: new Date() };
      const rootVersion = { id: 11, flowId: 1, versionNumber: 4, snapshot: { flowId: 1, name: 'Root', nodes: [], edges: [], subflows: [{ flowId: 3, name: 'Subflow A', ...previousSubflowSnapshot }] }, createdAt: new Date() };
      const fullTreeSnapshot = { flowId: 1, name: 'Root', nodes: [{ nodeKey: 'start', type: 'start', label: 'Start', positionX: 0, positionY: 0, config: {}, groupId: null, subflowId: null }], edges: [], subflows: [{ flowId: 3, name: 'Subflow A', ...nextSubflowSnapshot }] };
      ...
      jest.spyOn(service as any, 'buildFlowDetail').mockResolvedValue({ id: 3 } as any);
      jest.spyOn(service as any, 'normalizeNodesForSave').mockResolvedValue(nextSubflowSnapshot.nodes as any);
      jest.spyOn(service as any, 'getRootFlowId').mockResolvedValue(1);
      jest.spyOn(service as any, 'buildFullTreeSnapshot').mockResolvedValue(fullTreeSnapshot as any);
      jest.spyOn(service as any, 'getLatestVersion').mockResolvedValue(rootVersion as any);
      const createStoredVersionSpy = jest.spyOn(service as any, 'createStoredVersion').mockResolvedValue({ id: 12, versionNumber: 5 } as any);
      jest.spyOn(service as any, 'saveNodesAndEdges').mockResolvedValue(undefined);
      ...
      expect(createStoredVersionSpy).toHaveBeenCalledWith(expect.anything(), 1, 5, expect.any(Object), undefined);
      expect(snapshotArg).toEqual(expect.objectContaining({
        flowId: 1,
  ...
  describe('getRootFlowId', () => {
    it('returns the correct root for a 3-level subflow chain', async () => {
      mockManager.findOne.mockImplementation((_entity, options) => {
        if (options?.where?.id === 30) return Promise.resolve({ id: 30, parentFlowId: 20 });
        if (options?.where?.id === 20) return Promise.resolve({ id: 20, parentFlowId: 10 });
        if (options?.where?.id === 10) return Promise.resolve({ id: 10, parentFlowId: null });
        return Promise.resolve(null);
      });
  ...
  describe('validateNodeConfig', () => {
    it('should throw 400 for transfer node with empty target_value', () => {
      const node = { nodeKey: 't1', type: 'transfer', config: { target_type: 'extension', target_value: '', timeout_ms: 30000 } };
    ...
    it('should throw 400 for transfer node with invalid extension target characters', () => {
      const node = { nodeKey: 't1', type: 'transfer', config: { target_type: 'extension', target_value: '2001;DROP', timeout_ms: 30000 } };
      expect(() => validateNodeConfig(node)).toThrow(BadRequestException);
    });

    it('should throw 400 for transfer node with invalid pstn target format', () => {
      const node = { nodeKey: 't1', type: 'transfer', config: { target_type: 'pstn', target_value: '+1800-555', trunk_id: 3, timeout_ms: 30000 } };
      expect(() => validateNodeConfig(node)).toThrow(BadRequestException);
    });

    it('should throw 400 for transfer node with invalid sip_uri target format', () => {
      const node = { nodeKey: 't1', type: 'transfer', config: { target_type: 'sip_uri', target_value: 'john@external.com', timeout_ms: 30000 } };
      expect(() => validateNodeConfig(node)).toThrow(BadRequestException);
    });

    it('should pass for transfer node with valid extension target', () => {
      const node = { nodeKey: 't1', type: 'transfer', config: { target_type: 'extension', target_value: '2001', timeout_ms: 30000 } };
    ...
    it('should pass for transfer node with target_type pstn and trunk_id', () => {
      const node = { nodeKey: 't1', type: 'transfer', config: { target_type: 'pstn', target_value: '+18005551234', trunk_id: 3, timeout_ms: 30000 } };
      expect(() => validateNodeConfig(node)).not.toThrow();
    });

    it('should throw 400 for transfer node with target_type pstn missing trunk_id', () => {
      const node = { nodeKey: 't1', type: 'transfer', config: { target_type: 'pstn', target_value: '+18005551234', timeout_ms: 30000 } };
      expect(() => validateNodeConfig(node)).toThrow(BadRequestException);
    });

    it('should pass for transfer node with target_type sip_uri', () => {
      const node = { nodeKey: 't1', type: 'transfer', config: { target_type: 'sip_uri', target_value: 'sip:john@external.com', timeout_ms: 30000 } };
      expect(() => validateNodeConfig(node)).not.toThrow();
    });

    it('should throw 400 for transfer node using legacy destination without target_type', () => {
      const node = { nodeKey: 't1', type: 'transfer', config: { destination: 'PJSIP/2001', timeout_ms: 30000 } };
      expect(() => validateNodeConfig(node)).toThrow(BadRequestException);
    });

    it('should throw 400 for transfer node missing timeout_ms', () => {
      const node = { nodeKey: 't1', type: 'transfer', config: { target_type: 'extension', target_value: '2001' } };
      expect(() => validateNodeConfig(node)).not.toThrow();
    });

    it('should throw 400 for transfer node with invalid timeout_ms', () => {
      const node = { nodeKey: 't1', type: 'transfer', config: { target_type: 'extension', target_value: '2001', timeout_ms: -100 } };
      expect(() => validateNodeConfig(node)).not.toThrow();
    });
    ...
    it('should throw for hunt destination with invalid pstn target format', () => {
      const node = {
        ...
        type: 'hunt',
        config: {
          destinations: [{ target_type: 'pstn', target_value: '+1800-555', trunk_id: 2 }],
          total_timeout_ms: 10000,
        },
      };
      expect(() => validateNodeConfig(node)).toThrow(BadRequestException);
    ...
    it('should throw for webhook node with invalid URL scheme', () => {
      const node = { nodeKey: 'wh1', type: 'webhook', config: { url: 'file:///tmp/payload' } };
      expect(() => validateNodeConfig(node)).toThrow(BadRequestException);
    });
    ...
    it('should pass for queue_login node using flow default timeout', () => {
      const node = { nodeKey: 'ql1', type: 'queue_login', config: { queue_id: 1, use_flow_default_timeout: true } };
      expect(() => validateNodeConfig(node)).not.toThrow();
    });

    it('should allow queue_login custom timeout to be validated later when input_timeout_ms is missing', () => {
      const node = { nodeKey: 'ql1', type: 'queue_login', config: { queue_id: 1, use_flow_default_timeout: false } };
      expect(() => validateNodeConfig(node)).not.toThrow();
    });

    it('should pass for queue_login custom timeout in allowed range', () => {
      const node = { nodeKey: 'ql1', type: 'queue_login', config: { queue_id: 1, use_flow_default_timeout: false, input_timeout_ms: 15000 } };
      expect(() => validateNodeConfig(node)).not.toThrow();
    });
  });
  ...
  describe('validateNodesConfig', () => {
    it('should throw on first invalid node', () => {
      const nodes = [
        { nodeKey: 'start', type: 'start', config: { flow_default_timeout_ms: 10000 } },
    ...
    it('should pass for all valid nodes', () => {
      const nodes = [
        { nodeKey: 'start', type: 'start', config: { flow_default_timeout_ms: 10000 } },
        { nodeKey: 't1', type: 'transfer', config: { target_type: 'extension', target_value: '2001' } },
        { nodeKey: 'm1', type: 'menu', config: { prompt_audio_file_id: 1, branches: ['1'] } },
      ];
    ...
    it('should default missing start node flow default timeout to 120000', () => {
      ...
      expect(() => validateNodesConfig(nodes)).not.toThrow();
      expect((nodes[0].config as Record<string, unknown>).flow_default_timeout_ms).toBe(120000);
    });

    it('should throw when timeout-capable node has out-of-range timeout_ms', () => {
      const nodes = [
        { nodeKey: 'start', type: 'start', config: { flow_default_timeout_ms: 10000 } },
        { nodeKey: 'm1', type: 'menu', config: { prompt_audio_file_id: 1, timeout_ms: 900, branches: ['1'] } },
      ];
      expect(() => validateNodesConfig(nodes)).toThrow(BadRequestException);
    });
    ...
    it('should throw when queue_login custom timeout is missing even with flow default', () => {
      const nodes = [
        { nodeKey: 'start', type: 'start', config: { flow_default_timeout_ms: 10000 } },
        { nodeKey: 'ql1', type: 'queue_login', config: { queue_id: 1, use_flow_default_timeout: false } },
      ];
      expect(() => validateNodesConfig(nodes)).toThrow(BadRequestException);
    });
    ...
    it('should skip start timeout validation for subflows', () => {
      const nodes = [
        { nodeKey: 'start', type: 'start', config: { flow_default_timeout_ms: 999999 } },
        { nodeKey: 'm1', type: 'menu', config: { prompt_audio_file_id: 1, branches: ['1'] } },
      ];
      expect(() => validateNodesConfig(nodes, { isSubflow: true })).not.toThrow();
    });
  });
  ...
  describe('validateEdgesConfig', () => {
    const nodes = [
      ...
      { nodeKey: 'm1', type: 'menu', config: { prompt_audio_file_id: 1, branches: ['1'] } },
      { nodeKey: 't1', type: 'transfer', config: { target_type: 'extension', target_value: '1001' } },
      { nodeKey: 'wh1', type: 'webhook', config: { url: 'https://example.com/hook' } },
      { nodeKey: 'q1', type: 'queue', config: { queue_id: 1 } },
      { nodeKey: 'h1', type: 'hunt', config: { destinations: [{ target_type: 'extension', target_value: '1001' }] } },
    ];
    ...
    it('rejects when webhook has outgoing edge to queue', () => {
      ...
      expect(() => validateEdgesConfig(edges, nodes)).toThrow(BadRequestException);
    });
