...
const TEMPLATE_PREVIEWS: Record<string, string> = {
  'Medical Clinic IVR Template': 'Start → Business Hours → Menu → Transfer → Play Audio → Transfer → Play Audio → Hangup',
  'Restaurant IVR Template': 'Start → Play Audio → Menu → Transfer → Play Audio → Transfer → Hangup',
  ...
  'Appointment Reminder': 'Start → Play Audio → Webhook → Get Digits → Menu → Play Audio → Hangup | Play Audio → Hangup',
  ...
  return (
    <div className={styles.page}>
      <div className={styles.pageHeader}>
        ...
        <button className={styles.importJsonHeaderButton} onClick={() => setImportJsonOpen(true)} type="button">
          Import JSON
        </button>
      </div>
      {importJsonOpen ? (
        <section className={styles.importJsonPanel}>
          <button aria-label="Close import panel" className={styles.panelCloseButton} onClick={closeImportJsonPanel} type="button">×</button>
          ...
          <button className={styles.formatToggle} onClick={() => setShowExpectedFormat((current) => !current)} type="button">
            <span className={`${styles.formatToggleIcon} ${showExpectedFormat ? styles.formatToggleIconOpen : ''}`}>▶</span>
  ...
  "edges": [
    { "sourceNodeKey": "start", "targetNodeKey": "play_audio-1", "branchKey": "default" }
          ...
          <div className={styles.panelActions}>
            <button className={styles.modalCancelButton} onClick={closeImportJsonPanel} type="button">Cancel</button>
            <button className={styles.modalImportButton} disabled={importJsonBusy} onClick={() => void handleImportJson()} type="button">
                ...
                items.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className={styles.descriptionCell}>
                      ...
                      <div className={styles.previewText}>{TEMPLATE_PREVIEWS[item.name] || 'Start → ...'}</div>
                    </td>
      ...
      <section className={styles.guideSection}>
        <button className={styles.guideToggle} onClick={() => setShowGuide((current) => !current)} type="button">
          <span className={`${styles.guideIcon} ${showGuide ? styles.guideIconExpanded : ''}`}>▶</span>
          <span>How to create a template</span>
        </button>
        {showGuide ? (
          <div className={styles.guideSteps}>
            <div className={`${styles.accordionItem} ${openGuideStep === 1 ? styles.accordionItemOpen : ''}`}>
              <button
              ...
              {openGuideStep === 1 ? (
                <div className={styles.accordionBody}>
                  <p>Build your flow in the Flow Builder. Add nodes, configure them, wire the connections, and give the flow a meaningful name. When ready, save the flow using the Save button in the top bar.</p>
                </div>
              ) : null}
            </div>
            <div className={`${styles.accordionItem} ${openGuideStep === 2 ? styles.accordionItemOpen : ''}`}>
              <button
                className={styles.accordionHeader}
              ...
              {openGuideStep === 2 ? (
                <div className={styles.accordionBody}>
                  <p>Open the saved flow in the Flow Builder. Use the export option in the top bar to download the flow as a JSON file. The exported file contains all nodes, edges, positions, and configuration — everything needed to recreate the flow.</p>
                </div>
              ) : null}
            </div>
            <div className={`${styles.accordionItem} ${openGuideStep === 3 ? styles.accordionItemOpen : ''}`}>
              <button
                className={styles.accordionHeader}
              ...
              {openGuideStep === 3 ? (
                <div className={styles.accordionBody}>
                  <p>Paste the exported JSON into the Import JSON panel on this page. The JSON must contain a <code>nodes</code> array and an <code>edges</code> array at the minimum. Once imported, the flow opens as a new draft in the Flow Builder for you to rename and save.</p>
                </div>
              ) : null}
            </div>
          </div>
