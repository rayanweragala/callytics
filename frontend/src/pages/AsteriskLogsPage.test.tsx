...
describe('AsteriskLogsPage', () => {
  ...
  it('renders without crashing with empty entries', async () => {
    renderWithRouter(<AsteriskLogsPage />);
  ...
  it('adds tooltips for truncated values and toggles inline message expansion on row click', async () => {
    ...
    await user.click(messageNode.closest('tr') as HTMLElement);
  ...
  it('triggers debounced fetch with search parameter', async () => {
    ...
    expect(api.listAsteriskLogs).toHaveBeenCalled();
  ...
  it('shows call drill-down banner from query params and clears it', async () => {
    const user = userEvent.setup();
    renderWithRouter(<AsteriskLogsPage />, { initialEntries: ['/logs?uniqueid=call-123&from=2026-04-24T10%3A00%3A00.000Z&to=2026-04-24T10%3A00%3A30.000Z&callerNumber=2001&destination=3333'] });
